...
require_once ("PositionOfTheLines.php");
class PDFMerger
{
	...
	public function __construct()
	{
		...
		require_once('tcpdf/tcpdi.php');
		$this->fpdi = new TCPDI;
        $this->potl = new PositionOfTheLines();
	 ...
	 * @return void
	 */
	public function addPDF($filepath, $pages = 'all', $pageData)
	{
		if(file_exists($filepath))
		{
			...
			$this->_files[] = array($filepath, $pages, $pageData);
		}
		else
		{
    ...
    $this->fpdi->SetPrintFooter(false);
		...
		foreach($this->_files as $file)
		{
			...
			else
			{
				...
				{
					if(!$template = $this->fpdi->importPage($page)): throw new exception("Could not load page '$page' in PDF '$filename'. Check that the page exists."); endif;
					$size = $this->fpdi->getTemplateSize($template);
					$orientation = ($size['h'] > $size['w']) ? 'P' : 'L';
                    ...
                    $this->addPagePada( $page, $pageData);
				}
			}
		}
		...
		{
            $this->fpdi->Output($outputpath, $mode);
			return true;
		}
		else
		{
    ...
    private function addPagePada( $page, $pageData){
        $allLinePositions = $this->potl->getPagePositions($page);
        foreach ($pageData as $lineid=>$text){
            if(isset($allLinePositions[$lineid])) {
                $textArray = $this->mb_str_split($text);
                $this->printLines($allLinePositions[$lineid], $textArray);
            }
        }
    }

    private function printLines( $lineids, $textArray){
        $inlineStartPosition = 0;

        foreach ($lineids as $index => $linePosition){
    ...
    private function printOneLine($linePosition , $textArray, $inlineStartPosition){
        ...
        for($i=0; $i < $stringLength; $i++){
            ...
            $this->fpdi->SetXY($linePosition->x + $i * $this->cellWigth + $this->cpd->get($symbol), $linePosition->y);
            $this->fpdi->Write(10, $symbol);
